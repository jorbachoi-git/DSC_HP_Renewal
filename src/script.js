...
async function loadMsdsList(keyword = "") {
  ...
  try {
    ...
    if (posts.length === 0) {
      tbody.innerHTML = '<tr><td colspan="5" style="padding:20px; text-align:center;">등록된 게시물이 없습니다.</td></tr>';
      return;
    }
    ...
    tbody.innerHTML = '<tr><td colspan="5" style="padding:20px; text-align:center; color:red;">목록을 불러오지 못했습니다.</td></tr>';
  }
}
...
window.viewMsdsPost = async function (id) {
    try {
        ...
        document.getElementById("msds-detail-title").innerHTML = sanitizeHTML(post.title);
        document.getElementById("msds-detail-author").innerText = "작성자: " + post.author_name;
        document.getElementById("msds-detail-date").innerText = "작성일: " + new Date(post.created_at).toLocaleDateString("ko-KR");
        document.getElementById("msds-detail-views").innerText = "조회수: " + post.view_count;
...
window.toggleInquiryDetail = function(id) {
    ...
    if (isVisible) {
        ...
        // 현재 활성화된 섹션 ID 가져오기
        const currentSection = document.querySelector('.section-content.active').id || 'home';
