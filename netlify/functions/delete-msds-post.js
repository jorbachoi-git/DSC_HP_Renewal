...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (!id) {
        return { statusCode: 400, headers, body: JSON.stringify({ error: "ID is required for deletion" }) };
    }

    const sql = neon(process.env.DATABASE_URL);

    await sql`DELETE FROM msds_posts WHERE id = ${id}`;
    ...
    console.error("Delete MSDS Post Error:", error);
    return {
      statusCode: 500,
      headers,
